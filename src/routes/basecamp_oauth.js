...
// ----------------------------------------------------------------------------
// بوابة الإعداد: المساران كانا مفتوحين للعالم في الإنتاج.
//
// وهما مساران لمرّة واحدة، والملف نفسه يقول «يُفضَّل تعطيلهما بعد حفظ
// القيمتين» ولم يكن شيء يفعل ذلك. فيُغلقان تلقائياً متى تمّ الإعداد
// (وُجد BASECAMP_REFRESH_TOKEN)، ويمكن فتحهما بـ BASECAMP_OAUTH_SETUP=1
// عند الحاجة لإعادة الربط.
// ----------------------------------------------------------------------------
oauth.use('/basecamp/*', async (c, next) => {
  const forced = String(c.env.BASECAMP_OAUTH_SETUP || '') === '1';
  if (c.env.BASECAMP_REFRESH_TOKEN && !forced) {
    return c.text(
      'مسار إعداد بيسكامب مغلق: التوكن محفوظ مسبقاً. ' +
        'لإعادة الربط اضبط BASECAMP_OAUTH_SETUP=1 مؤقتاً ثم أزِله.',
      404
    );
  }
  await next();
