...
const PUBLIC_PATHS = [
  ...
  // الخروج. مسجَّل بعد الوسيط في `index.js` — ووسيط Hono على `*` يسبق كل
  // ما يليه — فبلا ذكره هنا يُحرَس مسارُ خروجٍ يُسقط الجلسة التي يُحرَس بها:
  // من انتهى رمزه يُحوَّل إلى المركز ليدخل قبل أن يُسمح له بالخروج، ومن
  // نادى بـ`fetch` يأخذ ٤٠١ مكان أن تُغلق جلسته.
  '/auth/logout',
  ...
  /* ولا `‎/api/basecamp/callback` هنا — وكان.
     الحجّة المكتوبة له كانت «يستدعيه بيسكامب لا المستخدم، فلا كوكي جلسة
     معه»، وهي غير صحيحة: رجوعُ OAuth بنمط `web_server` تحويلةٌ يتبعها
     **متصفّح المستخدم** نفسه، لا نداءٌ من خادم بيسكامب. والكوكي يصل معه —
     `SameSite=Lax` تسمح به لأنه تنقّلٌ علويّ بـ`GET`.

     وتركُه عاماً ليس بلا ثمن: الصفحة تعرض `refresh_token` لبيسكامب بعد
     المبادلة. ومن بدأ الرحلة من `‎/api/basecamp/start` — وهو محميّ — داخلٌ
     أصلاً، فحمايةُ الرجوع لا تكسر شيئاً. */
];
